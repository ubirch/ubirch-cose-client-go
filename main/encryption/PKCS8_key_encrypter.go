...
type PKCS8KeyEncrypter struct {
	Secret []byte
	Crypto ubirch.Crypto
}

func NewPKCS8KeyEncrypter(secret []byte, crypto ubirch.Crypto) (*PKCS8KeyEncrypter, error) {
	if len(secret) != 32 {
		return nil, fmt.Errorf("secret length for AES-256 encryption must be 32 bytes (is %d)", len(secret))
	}
	return &PKCS8KeyEncrypter{
		Secret: secret,
		Crypto: crypto,
	}, nil
}
...
func (enc *PKCS8KeyEncrypter) Encrypt(privateKeyPem []byte) ([]byte, error) {
...
func (enc *PKCS8KeyEncrypter) Decrypt(encryptedPrivateKey []byte) (privateKeyPem []byte, err error) {
