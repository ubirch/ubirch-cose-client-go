...
const (
	GatewayTimeout        = 10 * time.Second // time after which the client sends a 504 response if no timely response could be produced
	ShutdownTimeout       = 15 * time.Second // time after which the server will be shut down forcefully if graceful shutdown did not happen before
